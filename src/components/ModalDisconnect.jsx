...
export const ModalDisconnect = ({ closeModalDisconnect }) => {
  const handleDisconnect = useSelector(state => state.client.clientDisconnect)
      ...
      <ClickAwayListener onClickAway={closeModalDisconnect}>
        <motion.div className="w-[330px] h-[380px] bg-white rounded-[10px] py-[50px] px-[30px] flex flex-col items-center justify-center">
          ...
          <motion.div className="flex">
            ...
            <Button onClick={() => handleDisconnect()} sx={{ width: '110px', padding: '8px 10px', color: 'white', margin: '10px 8px' }} color='secondary' variant="contained">Disconnect</Button>
          </motion.div>
        </motion.div>
      </ClickAwayListener>
    </motion.div>
